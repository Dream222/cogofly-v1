...
SCRIPT_HIDE_AUTO = \
    u"<script>$(document).ready(function() {\n" \
    u"var hideAuto=function() {\n" \
    u"    $('#btn-password').parent().closest('ul')" \
    u"        .find('li').off('click', hideAuto);\n" \
    u"    setTimeout(function(){ \n" \
    u"        $('.panel-toggle-message').slideUp();\n" \
    u"    }, 10000);\n" \
    u"};\n" \
    u"$('#btn-password').parent().closest('ul')" \
    u".find('li').on('click', hideAuto);\n" \
    u"$($('#edit-profile-form').find('.row')[0]).show();\n" \
    u"$('#profile-summary').hide();\n" \
    u"});\n" \
    u"</script>\n"

MESSAGE_BETA_TESTEUR = [
    _(u"Thank you for your registration and welcome aboard!"),
    _(u'<p style="margin-bottom: -34px;">Take a few clicks to fill out, '
      u'confirm your practical information and continue the adventure.</p>'),
    SCRIPT_HIDE_AUTO,
    _(u'<p style="margin-bottom: -34px;">'
      u'<b>Don’t forget that in order for Cogofly members to find and '
      u'notice you via their different searches, you need to add '
      u'your travel plans: trips, weekends and days out...</b></p>'),
    u"<script>$(document).ready(function() { "
    u"$('#profile-btn-edit').click();"
    u"});</script>",
...
MESSAGE_THANKS_FOR_REGISTER_CHANGE_PASSWORD = [
    _(u'Thanks for registering!'),
    _(u'You have been successfully registered.'),
    _(u'Please <strong>change your password</strong>.'),
    _(u'And then you can complete your profile.'),
...
def login(req, provider_name):
    ...
    def local_log_then_redirect(user_req, ex):
        ...
        def exception_infos():
            ...
            return u'Exception in ({}, line {} "{}"): {}'.format(
                filename, lineno, line.strip(), exc_obj)

        LogTemporaire.write_log(u"An exception of type {} occured. ".format(
                                    type(ex).__name__))
        LogTemporaire.write_log(u'username = user_req.name = '
                                u'{}'.format(user_req.name))
        LogTemporaire.write_log(u'email = user_req.email = '
                                u'{}'.format(user_req.email))
        LogTemporaire.write_log(u'password = user_req.id = '
                                u'{}'.format(user_req.id))
        EmailMessage(
            subject=u"An exception of type {} occured. ".format(
                        type(ex).__name__).encode('utf_8'),
            body=u'{}\n{}\n{}\n{}\n'.format(
                u'username = user_req.name  = {}'.format(user_req.name),
                u'email    = user_req.email = {}'.format(user_req.email),
                u'password = user_req.id    = {}'.format(user_req.id),
                exception_infos()
            ).encode('utf_8'),
            from_email=u'contact@cogofly.com',
            to=[u'cogofly+error@gmail.com']).send()
        auth.logout(req)
    ...
    def local_create_user(user_req):
        try:
            ...
            while User.objects.filter(username=username).count() > 0:
                loop += 1
                username = u'{}_{}'.format(user_req.name, loop)

            retour = User.objects.create_user(
                username=username,
                email=user_req.email,
                password=user_req.id,
                last_name=user_req.last_name if user_req.last_name else u'?',
                first_name=user_req.first_name if user_req.first_name else u'?',
            )
            retour.save()

            if loop:
                EmailMessage(
                    subject=_(u'Login error: username already exists'),
                    body=_(u'He/she tried to login but '
                           u'username was already taken, so we changed it to: '
                           u'{}.\nHis/her email is: {}').format(
                        username, user_req.email
                    ),
                    from_email=u'contact@cogofly.com',
                    to=[u'cogofly+error@gmail.com', ],
                    bcc=[]).send()

        except IntegrityError as ex:   # column username is not unique
            req.session['message'] = [
                _(u'Technical error'),
                _(u"You tried to connect with Facebook or Google, but the "
                  u"mail was already registered without Facebook or google."),
                _(u"Try to login by entering mail and password below, "
                  u"without using Facebook or Google"),
                _(u'Click here to hide this message')]
            ...
            req.session['message'] = [
                _(u'Technical error'),
                _(u"You tried to connect with Facebook or Google."),
                _(u"There was a technical error with Facebook or Google."),
                _(u"We're sorry for this."),
                _(u"Please try again to login."),
                _(u'Click here to hide this message')]
            return local_log_then_redirect(user_req, ex)
        ...
        if user_req.content is not None:
            try:
                ...
                # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                LogTemporaire.write_log(u'BUG JSON !!')
        ...
        Personne.objects.create(
            user=retour,
            est_physique=True,
            champs_supplementaires=infos if infos is not None else u'',
    ...
    if result:
        ...
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        LogTemporaire.write_log(u'redirect -> {}'.format(redirect))
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        if result.error:
            auth.logout(req)
            response.write(
                u'<h2>Erreur #1 : {}</h2>'.format(result.error.message)
            )

            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            LogTemporaire.write_log(u'<h2>Erreur #2 : {}</h2>'.format(
            ...
            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            LogTemporaire.write_log(u'user vaut quelque chose : {}'.format(
                str(user)))
            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            if not user.name and not user.id and not user.email \
                ...
                req.session['message'] = [
                    _(u'No name'),
                    _(u"You tried to connect but did'nt precise a name"),
                    _(u"We're sorry but we need at least an email and a name "
                      u"to go further."),
                    _(u'Click here to hide this message')]
                return HttpResponseRedirect(reverse('my_home_error'))
            elif not user.id:
                auth.logout(req)
                raise Exception(u"Erreur d'authentification : aucun id")
            elif not user.email:
                auth.logout(req)
                req.session['message'] = [
                    _(u'No email'),
                    _(u"You tried to connect but did'nt precise an email"),
                    _(u"We're sorry but we need at least an email to "
                      u"go further."),
                    _(u'Click here to hide this message')]
                return HttpResponseRedirect(reverse('my_home_error'))
                ...
                try:
                    ...
                    try:
                        if isinstance(user_db, (list, tuple)):
                            if len(user_db):
                                for u in user_db:
                                    LogTemporaire.write_log(u'USER IS A LIST!!')
                        ...
                        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        LogTemporaire.write_log(u'user:')
                        LogTemporaire.write_log(str(user))
                        LogTemporaire.write_log(u'user_db:')
                    ...
                    if not p.user.is_active:  # Activer qu'une fois
                        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        LogTemporaire.write_log(u'? invité mais connect '
                                                u'DIRECT !')
                        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    ...
                    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    LogTemporaire.write_log(u'local_authenticate() : {}'.format(
                        str(user_db)))
                    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    ...
                    if redirect is not None:
                        try:
                            ...
                            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            LogTemporaire.write_log(u'on entre dans le hack')
                            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            ...
                            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            LogTemporaire.write_log(u'redirect : {}'.format(
                                redirect))
                            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            ...
                            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            LogTemporaire.write_log(u'on ne fait rien !')
                            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    ...
                    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    LogTemporaire.write_log(u'noter que "reg_already_done"')
                    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    ...
                    if result is not None:
                        ...
                        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        LogTemporaire.write_log(u'new user : {}'.format(str(p)))
                        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        ...
                        if redirect is None:
                            ...
                            LogTemporaire.write_log(
                                u'redirect OK + MESSAGE_BETA_TESTEUR '
                                u'my_home_profile_edit')
                            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            ...
                            if r.view_name.startswith('my_home_'):
                                ...
                                # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                                LogTemporaire.write_log(
                                    u'redirect 222 vers {}'.format(r.view_name))
                                # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                                # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            else:
                        ...
                        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        LogTemporaire.write_log(
                            u'au final, redirect = {}'.format(redirect))
                        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                if user_db is not None:
                    response.write(u'<h1>Found : {}</h1>'.format(user_db))
                    if user_db.is_active:
                        ...
                        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        LogTemporaire.write_log(
                            u'redirect PAS beta : {}'.format(redirect or u'/'))
                        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        return HttpResponseRedirect(redirect or u'/')
                    else:
                        response.write(u'Compte désactivé')
        else:  # User is none -> aucune authentification
            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            LogTemporaire.write_log(
                u'User is none -> aucune authentification ! WTF')
            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            return HttpResponseRedirect(redirect or u'/')
    return response
