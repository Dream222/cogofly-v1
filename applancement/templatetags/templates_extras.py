...
def joinby(arg1, arg2):
    ...
    if len(arg1) < 2:
        return arg1
    l = len(arg1)-1
    return _(u'{} and {}').format(arg2.join([str(p) for p in arg1[:l]]),
                                  str(arg1[l]))
...
def url_replace_parameter(url, **kwargs):
    ...
    for key, val in kwargs.iteritems():
        if isinstance(val, list):
            ...
            r[key] = [val]
    retour = u''
    for k, tab in r.iteritems():
        if len(tab) == 1:
            retour += u'{}={}'.format(k, tab[0])
        else:
            retour += u'&'.join([u'{}={}'.format(k, val) for val in tab])
        retour += u'&'
    return u'{}?{}'.format(u.path, retour)
