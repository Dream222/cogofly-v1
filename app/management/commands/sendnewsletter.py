    ...
    def send_newsletter(self, personne, nb_mails_sent, fake):
        ...
        for b in blogs_to_send:
            content = markdown_safe(b.content).replace(
                u'<img ', u'<img style="max-width: 100%;" ').replace(
                u'[HTML_REMOVED]', u'')
            append_m(u'',
                     u'<h1>{}</h1>{}'.format(b.title, content),
                     with_newline=False)
        ...
        if n > 0:
            m = ungettext(u'You have {} message not read.',
                          u'You have {} messages not read.', n).format(n)
            append_m(m)
        n = self.messages_not_read(personne).count()
        if n > 0:
            m = ungettext(u'You have {} invitation not read.',
                          u'You have {} invitations not read.', n).format(n)
            append_m(m)
        n = self.likes(personne).count()
        if n > 0:
            m = ungettext(u'You have {} like!',
                          u'You have {} likes!', n).format(n)
            append_m(m)

        # only add if something to say:
        if len(message):
            # !only add before anything else (position=0)
            append_m(str(_(u"Here is the latest news on your contacts, "
        ...
        if total_new > 50:
            append_m(str(_(u"Since the previous newsletter:")))
            if first_time:
                append_m(
                    msg=str(_(u"{} people are already "
                              u"registered!").format(total)),
                    html_msg=str(_(u"<b>{}</b> people are "
                                   u"already registered!").format(total))
                )
            else:
                append_m(
                    msg=str(
                        _(u"There have been {} new people registered!\n"
                          u"There's a total of <b>{}</b> "
                          u"people registered!").format(total_new, total)),
                    html_msg=str(
                        _(u"There have been <b>{}</b> new people registered!"
                          u"<br/>"
                          u"There's a total of <b>{}</b> "
                          u"people registered!").format(total_new, total))
                )
            append_m(str(
                _(u"There's a total of <b>{}</b> travels!\n").format(
            ...
            append_m(str(_(u"Here are some new travels that have been added:")),
                     with_newline=True)
        ...
        if total > 10:  # min. 10 searches (should be always more!)
            append_m(str(_(u"Here are some travels searches "
                           u"that have been done:")),
                     with_newline=False)
            ...
            for gmaps_id, cnt in random.sample(gmaps_ids_final, min(20, total)):
                ...
                append_m(str(_(u'- {} ({} results)')).format(
                    search_travel.formatted_address,
                    cnt),
                    html_msg=str(_(u'- <a href="{}?travel={}" target="_blank">'
                                   u'{}'
                                   u'</a>'
                                   u' ({} results)')).format(
                        u"{}{}".format(personne.site_web,
