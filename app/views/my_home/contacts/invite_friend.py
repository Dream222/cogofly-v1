...
class InviteFriendView(LoginRequiredMixin, generic.FormView):
    ...
    def get_initial(self):
        ...
        if msg:
            ...
            msg = _(u'Hi,\n\n'
                    u'Join me absolutely free on, www.cogofly.com, the new '
                    u'community where you can find people you can trust and '
                    u'never have to worry about travelling alone again.\n\n'

                    u'Don\'t stay on your own...meet people close to home or '
                    u'abroad and share your activities with them.\n\n'

                    u'Weekends away, days out, even business trips…you can '
                    u'envisage doing all of these with someone who potentially '
                    u'shares the same socio-professional criteria, hobbies and '
                    u'travel aspirations as you.\n\n'

                    u'If you enjoy spending time on your own…you will no doubt '
                    u'be inspired by other people’s experiences and be able to '
                    u'share information via this new web platform... and why '
                    u'not make the most of people’s recommendations.\n\n'
                    u'Cogofly Team : Alone, we think...'
                    u'Together, we get away!\n\n'

                    u'PS. Once you’ve checked out the site and finalized your '
                    u'subscription, please feel free to share this information '
                    u'with, maybe, your entourage.\n\n')
        initial['message'] = msg
        return initial
    ...
    def form_valid(self, form):
        ...
        if len(User.objects.filter(username__exact=username)):
            err = _(u'A user has already the same firstname and lastname')
        ...
        if len(User.objects.filter(email__exact=email)):
            form.add_error('email', _(u'This email is already used'))
            return super(InviteFriendView, self).form_invalid(form)
        ...
        if 'development' not in self.request.META['HTTP_HOST']:
            ...
            invite = u' '.join([self.request.user.first_name,
                                self.request.user.last_name]).strip()
            if invite:
                invite = _(u'{} has invited you!').format(invite)
            else:
                invite = _(u"Cogofly invites you!")

            email_subject = u'{} {}'.format(
                u'{}, '.format(u' '.join([nom, prenom]).strip())
                if prenom or nom else u'',
                invite
            ).strip()
        ...
        self.request.session['message'] = [
            _(u'Thanks for sharing!'),
            _(u'An email has been sent to the email address you provided.'),
