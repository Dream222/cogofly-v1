...
class SearchView(LoginRequiredMixin, generic.TemplateView):
    ...
    def clean_html(chaine, default=None):
        s = MLStripper()
        s.feed(chaine if chaine else u'')
        s = s.get_data().replace(u'\n', u' ').replace(u'\r', u'').strip()
    ...
    def get_context_data(self, **kwargs):
        ...
        g = self.request.GET
        travel = self.clean_html(g.get('travel'), u'')
        s_dd = self.clean_int(self, g.get('date_start_1'), u'**')
        s_mm = self.clean_int(self, g.get('date_start_0'), u'**')
        s_yy = self.clean_int(self, g.get('date_start_2'), u'-1')
        e_dd = self.clean_int(self, g.get('date_end_1'), u'**')
        e_mm = self.clean_int(self, g.get('date_end_0'), u'**')
        e_yy = self.clean_int(self, g.get('date_end_2'), u'-1')
        ...
        def mk_datepartial(yy, mm, dd):
            if yy != u'-1':
                ok = u'{:0>4}-{:0>2}-{:0>2}'.format(yy, mm, dd)
                try:
        ...
        if (s_dd != u'**') and (s_mm == u'**'):
            s_dd = u'**'
        if (e_dd != u'**') and (e_mm == u'**'):
            e_dd = u'**'
        ...
        age = self.clean_int(self, g.get('age'), u'-1')
        ...
        if travel:
            ...
            if len(v) == 0:
                ...
                try:
                    print u'{} not found -> asking google...'.format(travel)
                    ...
                    print u'cache done not we have:'
                    print u' // '.join([str(p) for p in v])
                except GoogleException as e:
                    self.request.session['message'] = [
                        _(u"Nothing found"),
                        _(u"Google didn't find a town/country with this name"),
                        _(u"Click here to hide this message")]
        ...
        if results is not None:
            ...
            context['search_results_title'] = _(
                u'Results matching with your criterias '
                u'found: <b>{}</b>').format(results.count())
        ...
        context['titre'] = _(u'Search')
        # reconstruire la forme en ré-injectant les données du GET :
        context['form_basique'] = SearchBasicForm({
            'travel': travel if travel else u'',
            'date_start': u'{}-{}-{}'.format(s_yy, s_mm, s_dd),
            'date_end': u'{}-{}-{}'.format(e_yy, e_mm, e_dd),
        })
        # Ajouter une erreur :
