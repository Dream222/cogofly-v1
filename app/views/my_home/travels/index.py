    ...
    def form_valid(self, form):
        ...
        if d_end and d_start:
            ...
            if d_end.date < d_start.date:
                form.add_error('date_start', _(u'Start is after date end...'))
                form.add_error('date_end', _(u'...End is before date starts'))
                # add_error supprime les champs de cleaned_data, les remettre :
                form.cleaned_data.update({'date_start': d_start,
                                          'date_end': d_end})
                return super(IndexView, self).form_invalid(form)
        ...
        if pk and not (travel or comments or d_start or d_end):
            ...
            try:
                ...
                pt.save()
                message = _(u'Your travel has been removed')
            except PersonneTravel.DoesNotExist:  # Hack devrait jamais arriver
                return super(IndexView, self).form_invalid(form)
        elif not travel:
            form.add_error('travel', _(u"Travel is mandatory"))
            return super(IndexView, self).form_invalid(form)
        elif pk:  # Mise à jour :
            try:
                pt = PersonneTravel.objects.get(
                ...
                pt.save()
                message = _(u'Your travel has been updated!')
            except PersonneTravel.DoesNotExist:  # Hack devrait jamais arriver
                return super(IndexView, self).form_invalid(form)
        else:
            ...
            message = _(u'Your travel has been added!')
        self.request.session['message'] = [
            _(u'Operation successful!'), message,
