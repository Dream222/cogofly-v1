...
class DateSelectorWidget(widgets.MultiWidget):
    ...
    def __init__(self, attrs=None):
        ...
        def custom_list(deb, fin, reverse=False):
            if reverse:
                ...
                b = range(deb, fin+1)
            b = zip(b, [unicode(c).rjust(2, '0') for c in b])
            b.insert(0, (u'**', u'-'))
            return b
    ...
    def decompress(self, value):
        ...
        if type(value) in [str, unicode]:
            try:
                ...
                yy, mm, dd = value.split(u'-')
                # Test date existante sinon ValueError :
                datetime(year=int(float(yy)) if yy != u'**' else 2000,
                         month=int(float(mm)) if mm != u'**' else 1,
                         day=int(float(dd)) if dd != u'**' else 1)
            except ValueError:
    ...
    def value_from_datadict(self, data, files, name):
        ...
        try:
            mm = int(datelist[0]) if datelist[0] != u'**' else None
            dd = int(datelist[1]) if datelist[1] != u'**' else None
            # print('dd', dd)
            # print('mm', mm)
            # print('ok, tentative...')
