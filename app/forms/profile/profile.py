...
class ProfileForm(SpecialTagTypedChoiceField, FormForceLocalizedDateFields):
    ...
    a = u'{}<span class="important-field">*</span>'.format(_(u'Sex:'))
    sexe = forms.IntegerField(
        label=a,
        widget=forms.Select(attrs={
            #  'title': a, <- pas échappé -> pas mettre
            'groupno': 2,
            'class': 'form-control'},
    ...
    a = _(u'Date of birth: <span class="important-field">*</span>')
    ...
    a = _(u'Mother tongue: <span class="important-field">*</span>')
    ...
    a = _(u'How did you know Cogofly? '
          u'<span style="font-size: smaller">'
          u'Important: a Cogoflyer may be close...'
          u'</span>')
    ...
    def __init__(self, *args, **kwargs):
        ...
        field_picture = forms.ImageField(
            ...
            widget=forms.FileInput(attrs={
                'title': a,
                'placeholder': _(u'my picture'),
                'accept': "image/*",
                'groupno': 0,
                'class': 'form-control', }),
            error_messages=self.e)

        a = _(u'Banner/background picture:')
        field_picture_banner = forms.ImageField(
            label=a, allow_empty_file=True, required=False,
            widget=forms.FileInput(attrs={
                'title': a,
                'placeholder': _(u'my banner'),
                'groupno': 1,
                'accept': "image/*",
                'class': 'form-control', }),
            error_messages=self.e)
        ...
        a = _(u'Town/Country of birth')
        field_place_of_birth = forms.CharField(
            label=a, max_length=100,
            widget=GoogleMapsWidget(attrs={
                'title': a, 'size': 100, 'type': 'text',
                'placeholder': _(u'my town/country of birth'),
                'groupno': 2,
                'class': 'form-control'}),
            error_messages=self.e,
            required=False)

        a = _(u'Town/Country where I live:') + u' <span ' \
                                               u'class="important-field">' \
                                               u'*</span>'
        field_place_i_live = forms.CharField(
            ...
            widget=GoogleMapsWidget(attrs={
                #  'title': a, <- pas échappé -> pas mettre
                'size': 100, 'type': 'text',
                'placeholder': _(u'town where I live'),
                'groupno': 2,
                'class': 'form-control'}),
            error_messages=self.e)

        a = _(u'Current employer:')
        field_employer_current = forms.CharField(
            label=a, max_length=100, required=False,
            widget=GoogleMapsWidget(attrs={
                'title': a, 'size': 100, 'type': 'text',
                'placeholder': _(u'current employer'),
                'groupno': 3,
                'rowstart': True,
                'rowspan': 6,
                'class': 'form-control'}),
        ...
        a = _(u'Previous employer:')
        field_employer_previous = forms.CharField(
            label=a, max_length=100, required=False,
            widget=GoogleMapsWidget(attrs={
                'title': a, 'size': 100, 'type': 'text',
                'placeholder': _(u'previous employer'),
                'groupno': 3,
                'rowspan': 6,
                'rowend': True,
                'class': 'form-control'}),
        ...
        programmes2 = TagTypedChoiceField(
            ...
            widget=CheckboxSelectMultipleBootstrap(attrs={
                'title': a,
                'groupno': 3,
                'multiple': 'multiple',
                'rowspan': 6,
        ...
        if retour:
            try:
                ...
                self.errors['field_picture'] = \
                    ErrorList([_(u"Unknown type of image")])
        return retour
    ...
    def clean_date_naissance(self):
        ...
        if not date_naissance:
            raise ValidationError(_(u'No birth date precised'))

        if relativedelta(datetime.datetime.now(), date_naissance).years < 18:
                raise ValidationError(_(u'Invalid birth date '
                                        u'(you must be >= 18 years old)'))
    ...
    def clean(self):
        ...
        try:
            ...
            raise ValidationError(
                _(u"The language you've set is unknown ('{}'). "
                  u"Please go to My Profile -> Change my parameters "
                  u"and choose a known language").format(
