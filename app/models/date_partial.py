...
class DatePartial(object):
    ...
    def __init__(self, year, mm=None, dd=None):
        if not year:
            raise AssertionError(_(u"Year can't be None"))
        if dd and (not mm):
            raise AssertionError(_(u"Day must be None if month is None"))
    ...
    def __str__(self):
        return u'{:0>4}-{:0>2}-{:0>2}'.format(
            self.date.year,
            self.date.month if not self.ignore_mm else u'**',
            self.date.day if not self.ignore_dd else u'**')
    ...
    @property
    def month(self):
        return self.date.month if not self.ignore_mm else u'**'

    @property
    def day(self):
        return self.date.day if not self.ignore_dd else u'**'
